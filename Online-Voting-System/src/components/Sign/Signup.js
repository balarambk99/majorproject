...
export default function Signup() {
    ...
    const stringToBase64 = async (file) => {
    ...
    const handleFileChange = async (e) => {
        ...
        try {
            const base64String = await stringToBase64(e.target.files[0]);
            ...
            console.log(formData.image); // This may still log the old state due to state update being asynchronous
    ...
    const verifyOtp = async () => {
        ...
        try {
            const response = await axios.post(`${BASE_URL}/auth/verify-otp`, { email: formData.email, otp });
